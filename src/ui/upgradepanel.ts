...
// Accessible via [U] ou bouton HUD.
// ============================================================

import { ALL_UPGRADES, UpgradeSystem, type UpgradeDef } from '../core/upgrades';
...
export function initUpgradePanel(
  ...
  panel.addEventListener('click', (e) => {
    if (e.target === panel) hideUpgradePanel();
  });
}
  ...
  for (const upg of ALL_UPGRADES) {
    ...
    item.innerHTML = `
      <div class="up-top">
        ...
        <span class="up-cost ${!affordable && !bought ? 'up-cant' : ''}">
          ${bought ? '' : `${upg.cost} pts`}
        </span>
      </div>
...
export function toggleUpgradePanel(): void {
  visible ? hideUpgradePanel() : showUpgradePanel();
}

export function showUpgradePanel(): void {
  visible = true;
  const panel = document.getElementById('goetia-upgrades');
  if (panel) { panel.style.display = 'flex'; renderUpgradePanel(); }
}
