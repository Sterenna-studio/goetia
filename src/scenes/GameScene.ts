...
export class GameScene extends Phaser.Scene {
  ...
  private _showGameOver(): void {
    ...
    this.add.text(640, 260, 'GAME OVER', { fontSize: '64px', color: '#cc4444', fontFamily: 'monospace', fontStyle: 'bold' }).setOrigin(0.5).setDepth(11);
    this.add.text(640, 350, `Score : ${this.sim.score}`, { fontSize: '32px', color: '#ffffff', fontFamily: 'monospace' }).setOrigin(0.5).setDepth(11);
    this.add.text(640, 400, `Vague atteinte : ${this.sim.waveSystem.currentWave}`, { fontSize: '20px', color: '#88ccff', fontFamily: 'monospace' }).setOrigin(0.5).setDepth(11);
    this.add.text(640, 460, `Upgrades achetées : ${this.sim.upgrades.getPurchased().length}`, { fontSize: '16px', color: '#aa9966', fontFamily: 'monospace' }).setOrigin(0.5).setDepth(11);
    this.add.text(640, 510, '[R] Recommencer', { fontSize: '22px', color: '#ffaa44', fontFamily: 'monospace' }).setOrigin(0.5).setDepth(11);
  }
  ...
  private _render(): void {
    ...
    for (const enemy of this.world.enemies.values()) {
      ...
      g.fillStyle(0x333333); g.fillRect(enemy.pos.x - 12, enemy.pos.y - 18, 24, 4);
      g.fillStyle(0xcc4444); g.fillRect(enemy.pos.x - 12, enemy.pos.y - 18, 24 * hpRatio, 4);
    }
  }
}
